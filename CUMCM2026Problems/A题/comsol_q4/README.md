...
3. 点击工具栏“绘制”。模型已经包含覆盖第四问新达标时刻的瞬态解，无需重新计算即可查看。
...
- 圆柱侧面：`hT=25 W/(m²·K)`，`hm=8e-7 m/s`；环境函数来自附件 1，4 h 后切换为恒温工艺设定值 50 °C、0.05 kg/kg。
- 收缩解释：均匀材料收缩，固定材料坐标内无相对对流项，扩散项和边界通量按 `R0/R(t)` 缩放。

## 主要结果

- 主模型给定的第四问判据时刻：51.0876993 h；这里比较该时刻的浓度，未独立求解 COMSOL 首达时间。
- 此时半径：1.200 cm。
- COMSOL 中心含水率：0.150036 kg/kg；表面含水率：0.052191 kg/kg。
- 与完整 161 节点有限体积解在六个相同时刻、101 个径向点比较，最大含水率绝对差为 0.011617 kg/kg，位于 24 h、r/R=0.99；最大温度绝对差为 0.000069 °C。近表层偏差仍需进一步网格检验。
